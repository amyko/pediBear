...
public class PairwiseLikelihoodCoreStream2 {
	...
	private double transitionDensityBilineal(int ibdNew, int ibdOld, double rhoBetween, Relationship rel){
		...
		double p11 = -1d;
		if((path.getUp() > path.getDown() && path.getDown()==1) || (path.getDown() > path.getUp() && path.getUp()==1)){ //uncle-type relationship
			//p11 = Math.pow(1-rhoBetween, m-1) * (.5*(1-rhoBetween) + .5 + 2*rhoBetween);
