...
public class PairwiseLikelihoodCoreStreamPed {
	...
	private static final int pA = 7;
	private static final int pC = 8;
	private static final int pG = 9;
	...
	public PairwiseLikelihoodCoreStreamPed(double genotypingErrorRate, double recombRate, int back, int numIndiv){
	...
	public double[] computeMarginal(String genoPath, String infoPath, int[] ids) throws IOException{ //works
			...
			if(currChrom!=prevChrom){
				...
				for (int ibdNew = 0; ibdNew < numIBD; ibdNew++){
					...
					for(int i=0; i<numIndiv; i++){
						...
						for(int j=i+1; j<numIndiv; j++){
							int i2 = 2*indCols[j]+4;
							EmissionKey key = new EmissionKey(new SimplePair<Genotype,Genotype>(new Genotype(ldGeno[i1]+ldGeno[i1+1]), new Genotype(ldGeno[i2]+ldGeno[i2+1])), new SimplePair<Genotype,Genotype>(new Genotype(geno[i1]+geno[i1+1]), new Genotype(geno[i2]+geno[i2+1])), ibdNew);
